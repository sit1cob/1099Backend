...
import { PartPhotoModel } from '../models/partPhoto';
import mongoose from 'mongoose';
import { ExternalApiAdapter, EXTERNAL_API_URL } from '../services/externalApiAdapter';

export const vendorsRouter = Router();

// Multer configuration for part photo uploads
const upload = multer({
...
// POST /api/vendors/parts/:partId/photos - NO AUTH (proxies to external API)
// Upload photos for parts documentation
// This must be defined BEFORE the authenticateJWT() middleware
vendorsRouter.post('/parts/:partId/photos', upload.array('photos', 10), async (req: AuthenticatedRequest, res) => {
  try {
    const { partId } = req.params;
    const { description } = req.body;
    
    console.log('[UploadPartPhotos] ========================================');
    console.log('[UploadPartPhotos] Calling EXTERNAL API:', `${EXTERNAL_API_URL}/api/vendor/parts/${partId}/photos`);
    console.log('[UploadPartPhotos] Part ID:', partId);
    console.log('[UploadPartPhotos] Description:', description);
    console.log('[UploadPartPhotos] Files count:', req.files ? (req.files as Express.Multer.File[]).length : 0);
    console.log('[UploadPartPhotos] ========================================');
    ...
    try {
      ...
      const externalResponse = await ExternalApiAdapter.uploadMultipartData(
        `/api/vendor/parts/${partId}/photos`,
        token,
        formData
      );
      
      console.log('[UploadPartPhotos] ========== EXTERNAL API RESPONSE ==========');
      console.log('[UploadPartPhotos] Response:', JSON.stringify(externalResponse, null, 2));
      console.log('[UploadPartPhotos] ================================================');

      // Save photo URLs to MongoDB
      const savedPhotos: any[] = [];
      if (externalResponse?.success && externalResponse?.data?.photos) {
        console.log('[UploadPartPhotos] Saving photos to MongoDB...');
        ...
        for (const photo of externalResponse.data.photos) {
          try {
            ...
            console.error('[UploadPartPhotos] Failed to save photo to MongoDB:', saveErr.message);
          }
        }
        
        console.log('[UploadPartPhotos] ✓ Saved', savedPhotos.length, 'photos to MongoDB');
      }
      ...
      console.log('[UploadPartPhotos] ✓ Returning combined response');
      return res.json(combinedResponse);
    } catch (extErr: any) {
      console.error('[UploadPartPhotos] ✗ External API call failed:', extErr.message);
    ...
    console.error('[UploadPartPhotos] Unexpected error:', err);
    return res.status(500).json({ success: false, message: err?.message || 'Failed to upload part photos' });
  }
});

// DELETE /api/vendors/parts/:partId/photos - NO AUTH (proxies to external API)
// Remove uploaded photos for a part
// This must be defined BEFORE the authenticateJWT() middleware
vendorsRouter.delete('/parts/:partId/photos', async (req: AuthenticatedRequest, res) => {
  try {
    const { partId } = req.params;
    const { photoIds } = req.body;
    
    console.log('[DeletePartPhotos] ========================================');
    console.log('[DeletePartPhotos] Calling EXTERNAL API:', `${EXTERNAL_API_URL}/api/vendor/parts/${partId}/photos`);
    console.log('[DeletePartPhotos] Part ID:', partId);
    console.log('[DeletePartPhotos] Photo IDs:', JSON.stringify(photoIds));
    console.log('[DeletePartPhotos] ========================================');
    ...
    if (!photoIds || !Array.isArray(photoIds) || photoIds.length === 0) {
      return res.status(400).json({ success: false, message: 'photoIds array is required and must not be empty' });
    }

    try {
      // Call external API to delete the photos
      const externalResponse = await ExternalApiAdapter.callExternalApi(
        `/api/vendor/parts/${partId}/photos`,
        token,
        'DELETE',
      ...
      console.log('[DeletePartPhotos] ========== EXTERNAL API RESPONSE ==========');
      console.log('[DeletePartPhotos] Response:', JSON.stringify(externalResponse, null, 2));
      console.log('[DeletePartPhotos] ================================================');

      // Delete photos from MongoDB if external API succeeded
      let deletedCount = 0;
      if (externalResponse?.success) {
        console.log('[DeletePartPhotos] Deleting photos from MongoDB...');
        
        try {
          // Delete by photoId if available, otherwise by partId
          const deleteResult = await PartPhotoModel.deleteMany({
            partId: parseInt(partId),
            photoId: { $in: photoIds }
          });
          deletedCount = deleteResult.deletedCount || 0;
          console.log('[DeletePartPhotos] ✓ Deleted', deletedCount, 'photos from MongoDB');
        } catch (deleteErr: any) {
          console.error('[DeletePartPhotos] Failed to delete photos from MongoDB:', deleteErr.message);
        }
      }

      // Return combined response with MongoDB deletion count
      const combinedResponse = {
        ...externalResponse,
        mongoDeleted: deletedCount,
      };

      console.log('[DeletePartPhotos] ✓ Returning combined response');
      return res.json(combinedResponse);
    } catch (extErr: any) {
      console.error('[DeletePartPhotos] ✗ External API call failed:', extErr.message);
    ...
    console.error('[DeletePartPhotos] Unexpected error:', err);
    return res.status(500).json({ success: false, message: err?.message || 'Failed to delete part photos' });
  }
});
